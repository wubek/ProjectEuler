...
def make_reverse_number(number):
	''' Ex. 178 -> 871 '''
	digits, reverse = map(lambda dig: ord(dig) - ord("0"), str(number)), 0
	for i in range(len(digits)):
		reverse += digits[i] * 10 ** i
	return reverse

def is_lychrel_num(num):
	iteration, rev = 1, make_reverse_number(num)
	print(num, rev)
	while iteration < 50:
		num += rev
		rev = make_reverse_number(num)
		if rev == num:
			return 0
		iteration += 1
	return 1
